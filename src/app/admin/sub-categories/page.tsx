...
export default function AdminSubCategoriesPage() {
  ...
  const handleFormSubmit = async (data: Omit<FirestoreSubCategory, 'id' | 'createdAt'> & { id?: string, slug?: string }) => {
    ...
    const payloadForFirestore: Partial<Omit<FirestoreSubCategory, 'id' | 'createdAt'>> = {
      name: data.name, 
      slug: finalSlugForSave, 
      parentId: data.parentId, 
      order: Number(data.order),
      isActive: data.isActive === undefined ? true : data.isActive, // Ensure isActive is always set
