...
import type { ThemeColors, ThemePalette } from '@/types/firestore';
...
import { updateThemeSettings, getThemeSettings } from './actions/theme-actions';
...
export default function ThemeSettings() {
  ...
  const loadThemeSettings = useCallback(async () => {
    setIsLoading(true);
    try {
      const themeColors = await getThemeSettings();
      if (themeColors) {
        ...
        colorSettingsGroups.flatMap(g => g.settings).forEach(config => {
          const lightHslValue = themeColors.light?.[config.id];
          ...
          const darkHslValue = themeColors.dark?.[config.id];
