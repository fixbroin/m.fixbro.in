...
import { PlusCircle, Edit, Trash2, ShoppingBag, CheckCircle, XCircle, Loader2, Percent, PackageSearch, MessageSquare, Filter, Wand2, Users, Star } from "lucide-react"; 
import type { FirestoreReview, ReviewStatus, FirestoreService, FirestoreSubCategory, FirestoreCategory, ProviderApplication } from '@/types/firestore';
...
import { collection, getDocs, addDoc, updateDoc, deleteDoc, doc, orderBy, query, Timestamp, onSnapshot, where } from "firebase/firestore";
...
export default function AdminReviewsPage() {
  ...
  useEffect(() => {
    ...
    const tryStopLoading = () => {
      if (prerequisitesLoaded && initialReviewsLoaded) {
        setIsLoading(false);
      }
    };
    ...
    const fetchPrerequisites = async () => {
        try {
            ...
            const provQuery = query(collection(db, "providerApplications"), where("status", "==", "approved"), orderBy("fullName"));
            
            const [servicesSnap, subCatsSnap, catsSnap, provSnap] = await Promise.all([
                getDocs(servicesQuery), getDocs(subCatsQuery), getDocs(catsQuery), getDocs(provQuery)
            ]);
            ...
            setProviders(provSnap.docs.map(doc => ({ ...doc.data(), id: doc.id } as ProviderApplication)));
        } catch (error) {
  ...
  const handleFormSubmit = async (data: ReviewFormData & { serviceName?: string; adminCreated: boolean; id?: string }) => {
    ...
    const payload: Partial<Omit<FirestoreReview, 'id'>> = {
    ...
    if (data.subjectType === 'service') {
      payload.serviceId = data.serviceId;
      payload.serviceName = data.serviceName;
    } else {
      ...
      <Card>
        <CardHeader className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          ...
          <div className="w-full sm:w-auto flex flex-col sm:flex-row gap-2">
            ...
            <Button onClick={() => setIsBulkGenerateOpen(true)} variant="outline" className="w-full sm:w-auto h-9" disabled={isLoading || providers.length === 0}>
                <Wand2 className="mr-2 h-4 w-4" /> AI Bulk Generate
            </Button>
            <Button onClick={handleAddReview} disabled={isSubmitting || isLoading || providers.length === 0} className="w-full sm:w-auto h-9">
                ...
                {filteredReviews.length === 0 ? (
                  ...
                  filteredReviews.map((review) => (
                    <TableRow key={review.id}>
                      <TableCell className="font-medium text-xs max-w-[150px] truncate" title={review.serviceName || review.providerId}>
                        {review.serviceName ? `Service: ${review.serviceName}` : `Provider: ${review.providerId.substring(0,8)}...`}
                      </TableCell>
                          ...
                          <AlertDialog>
                            ...
                            <AlertDialogContent>
                              <AlertDialogHeader><AlertDialogTitle>Are you sure?</AlertDialogTitle><AlertDialogDescription>
                                  This will permanently delete the review by "{review.userName}" for "{review.serviceName || `Provider ${review.providerId.substring(0,5)}...`}".
                                </AlertDialogDescription></AlertDialogHeader>
                              <AlertDialogFooter><AlertDialogCancel disabled={isSubmitting}>Cancel</AlertDialogCancel><AlertDialogAction
      ...
      <Dialog open={isFormOpen} onOpenChange={(open) => { if (!isSubmitting) { setIsFormOpen(open); if (!open) setEditingReview(null); } }}>
        <DialogContent className="w-[90vw] max-w-md sm:max-w-lg md:max-w-xl lg:max-w-2xl max-h-[90vh] p-0 flex flex-col">
          ...
          <div className="flex-grow overflow-y-auto"> {/* This div handles scrolling of the ReviewForm */}
            ...
            ) : (providers.length === 0) && !editingReview ? (
              <div className="p-6 py-8 text-center">
                  <p className="text-destructive">Cannot add new reviews because no approved providers exist.</p>
                  <p className="text-muted-foreground text-sm mt-2">Please approve at least one provider first.</p>
              </div>
            ) : (
              <ReviewForm
