...
export default function ProviderLayout({ children }: PropsWithChildren) {
  ...
  // This check determines if the current route is a private provider page or a public profile page.
  const privateProviderPaths = [
    '/provider/profile',
    '/provider/my-jobs',
    '/provider/quotation-invoice',
    '/provider/earnings',
    '/provider/withdrawal',
    '/provider/reviews',
  ...
  const isPrivateProviderPage = pathname === '/provider' || privateProviderPaths.some(p => pathname.startsWith(p)) || pathname.startsWith('/provider/booking');

  if (!isPrivateProviderPage) {
    // This is a public page, like /provider/[providerId].
    // Render without the provider panel, letting the root layout take over.
    return <>{children}</>;
  }

  // The rest of the file handles the private provider panel logic.
  useEffect(() => {
