...
import Step4LocationAndTerms from '@/components/provider-registration/Step4LocationAndTerms';
import { useApplicationConfig } from '@/hooks/useApplicationConfig';
import { ADMIN_EMAIL } from '@/contexts/AuthContext';
import { sendNewProviderApplicationAdminEmail, type NewProviderApplicationAdminEmailInput } from '@/ai/flows/sendNewProviderApplicationAdminNotificationFlow';
...
export default function ProviderRegistrationPage() {
  ...
  const fetchApplicationAndUserData = useCallback(async () => {
    ...
    try {
      ...
      if (userDocSnap.exists()) {
        ...
        setUserFirestoreData(fetchedUserDbData); 
      }
  ...
  const handleSaveStep = useCallback(async (stepData: Partial<ProviderApplication>) => {
    const targetUserIdForSave = editingApplicationIdForAdmin || user?.uid;
    if (!targetUserIdForSave) throw new Error("No user ID available to save data.");
    
    setIsSavingStep(true);
    const dataToUpdate = { ...applicationData, ...stepData };
    
    try {
      const appDocRef = doc(db, PROVIDER_APPLICATION_COLLECTION, targetUserIdForSave);
      await setDoc(appDocRef, { ...dataToUpdate, updatedAt: Timestamp.now() }, { merge: true });
  ...
  }, [applicationData, editingApplicationIdForAdmin, user?.uid]);
  ...
  const handleNextStep = async (stepDataFromComponent: Partial<ProviderApplication>) => {
    ...
    const updatedData = { ...applicationData, ...stepDataFromComponent, status: isEditModeByAdmin ? applicationData.status : nextProviderStepStatus };
  ...
  const handleFinalSubmitApplication = async (finalStepData: Partial<ProviderApplication>) => {
    ...
    delete (completeFinalData as any).bankDetails; // Explicitly remove bankDetails field
  ...
  const renderStepContent = () => {
    ...
    switch (currentStep) {
      ...
      case 4: return <Step4LocationAndTerms onSubmit={handleFinalSubmitApplication} onPrevious={handlePreviousStep} initialData={initialStepDataForForm} isSaving={isSavingStep} userUid={editingApplicationIdForAdmin || user?.uid || ""} onSaveStep={handleSaveStep} />;
