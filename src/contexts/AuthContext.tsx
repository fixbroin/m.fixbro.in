...
interface AuthContextType {
  ...
  handleSuccessfulAuth: (userCredential: UserCredential, extraDetails?: Partial<SignUpData>) => Promise<void>;
  isCompletingProfile: boolean;
  userCredentialForProfileCompletion: UserCredential | null;
  pendingSignUpDetails: Partial<SignUpData> | null; // Pass signup form data to dialog
...
export const AuthProvider: React.FC<PropsWithChildren> = ({ children }) => {
  ...
  const [pendingSignUpDetails, setPendingSignUpDetails] = useState<Partial<SignUpData> | null>(null);
  ...
  const handleSuccessfulAuth = useCallback(async (userCredential: UserCredential, extraDetails?: Partial<SignUpData>) => {
    setIsLoading(true);
    ...
    try {
      ...
      if (!docSnap.exists()) {
        if (extraDetails) {
            setPendingSignUpDetails(extraDetails);
        }
      ...
      await runTransaction(db, async (transaction) => {
        ...
        if (referralCodeParam && referralSettings?.isReferralSystemEnabled) {
          ...
          if (existingReferralSnap.empty) {
            console.log(`No prior referral usage found. Proceeding.`);
            ...
            if (!referrerSnapshot.empty) {
              ...
              console.log(`Referrer found: ${referrerId}. Bonus: ${referredBonus}`);
              ...
              console.log("Referrer notification created.");
            } else {
              console.warn(`Referral code "${referralCodeParam}" not found.`);
            }
          } else {
              console.warn(`Potential repeat referral detected. Skipping bonus.`);
          }
        }
        ...
        console.log(`New user document for ${user.uid} created with balance ${initialWalletBalance}.`);
      });
  ...
  const signUp = useCallback(async (data: SignUpData) => {
    ...
    try {
      ...
      await handleSuccessfulAuth(userCredential, { fullName: data.fullName, mobileNumber: data.mobileNumber });
    } catch (error) {
      const authError = error as AuthError;
  ...
  const contextValue: AuthContextType = useMemo(() => {
    return {
      ...
      pendingSignUpDetails, // Expose pending details
      completeProfileSetup,
      cancelProfileCompletion,
      setUser,
    };
  }, [user, firestoreUser, isLoading, authActionRedirectPath, internalTriggerAuthRedirect, signUp, logIn, logOut, signInWithGoogle, handleSuccessfulAuth, isCompletingProfile, userCredentialForProfileCompletion, pendingSignUpDetails, completeProfileSetup, cancelProfileCompletion, setUser]);
