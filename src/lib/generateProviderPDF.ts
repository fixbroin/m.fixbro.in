...
async function getImageDataUri(url: string): Promise<{ dataUri: string; format: string } | null> {
  ...
  try {
    // This is a browser API. The function will fail in a pure server-side environment (e.g. Next.js build)
    // if not polyfilled or handled. Assuming this runs client-side.
...
export const generateProviderApplicationPdf = async (
  ...
  y = addDetail(doc, "Bio / About Me", application.bio, y);
  y += 5;
