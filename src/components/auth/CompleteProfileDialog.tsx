...
interface CompleteProfileDialogProps {
  ...
  pendingDetails: Partial<SignUpData> | null;
...
export default function CompleteProfileDialog({
  ...
  const form = useForm<ProfileFormData>({
    ...
    defaultValues: {
      fullName: pendingDetails?.fullName || userCredential.user.displayName || "",
      email: pendingDetails?.email || userCredential.user.email || "",
      mobileNumber: (pendingDetails?.mobileNumber || userCredential.user.phoneNumber || "").replace(config.defaultOtpCountryCode || '+91', ''),
    },
  });
  ...
  }, [userCredential, pendingDetails, config, form]);

  const handleSubmit = async (data: ProfileFormData) => {
    let mobileNumberForSubmit = data.mobileNumber;

    if (isGoogleSignIn || (!isPhoneSignIn && data.mobileNumber)) {
    ...
    if ((isPhoneSignIn || !isGoogleSignIn) && (!data.email || !z.string().email().safeParse(data.email).success)) {
      ...
      >
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-4">
            ...
            {(isGoogleSignIn || (!isGoogleSignIn && !isPhoneSignIn)) && (
              <FormField
                control={form.control}
                name="mobileNumber"
                render={({ field }) => (
            ...
            {(isPhoneSignIn || (!isGoogleSignIn && !isPhoneSignIn)) && (
              <FormField
                control={form.control}
                name="email"
                render={({ field }) => (
