...
export default function Step2PersonalInfo({
  ...
  useEffect(() => {
    ...
    setOriginalImageUrlFromInitialData(initialData.profilePhotoUrl || null);
  ...
  const handleFileSelected = (event: React.ChangeEvent<HTMLInputElement>) => {
    if (event.target.files && event.target.files[0]) {
      ...
      setSelectedFile(null);
      setCurrentImagePreview(form.getValues('profilePhotoUrl') || initialData.profilePhotoUrl || null);
    }
  };
    ...
    if (selectedFile) {
      ...
      try {
        ...
        try {
            ...
            console.warn("Old profile photo not deleted on clear:", e);
        }
  ...
  const effectiveIsSaving = isSaving || isFormBusyForImage;
      ...
      <form onSubmit={form.handleSubmit(handleSubmit)}>
        <CardContent className="space-y-6">
          ...
          <FormItem>
            ...
            <FormDescription>Upload any image size. It will be stored on the server.</FormDescription>
            {uploadProgress !== null && selectedFile && (
                <div className="mt-2"><Progress value={uploadProgress} className="w-full h-2" />{statusMessage && <p className="text-xs text-muted-foreground mt-1">{statusMessage}</p>}</div>
            )}
