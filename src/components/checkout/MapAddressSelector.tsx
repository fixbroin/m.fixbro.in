...
interface MapAddressSelectorProps {
  apiKey: string;
  onAddressSelect: (address: Partial<AddressFormData>) => void | Promise<void>;
  onClose: () => void;
...
const MapAddressSelector: React.FC<MapAddressSelectorProps> = ({ apiKey, onAddressSelect, onClose, initialCenter, serviceZones, onManualSearchClick }) => {
  ...
  useEffect(() => {
    if (mapInstanceRef.current && initialCenter) {
      ...
      mapInstanceRef.current.setZoom(DETAILED_ZOOM);
      updateMarker(initialCenter, mapInstanceRef.current, true, false);
    }
  }, [initialCenter, updateMarker]); // React to changes in initialCenter prop
