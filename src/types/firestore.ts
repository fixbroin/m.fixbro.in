...
export interface FirestoreReview {
  id: string; // Firestore document ID
  serviceId?: string; // Optional: The specific service if the review is from a booking
  serviceName?: string; // Denormalized, optional
  providerId: string; // ID of the provider being reviewed
  ...
  createdAt: Timestamp;
  updatedAt?: Timestamp;
  bookingId?: string; // Optional: The booking this review is associated with
}
...
export interface AppSettings {
  ...
  isComplaintSystemEnabled?: boolean; // New field for complaint system toggle

  // Provider Fee Settings
  providerFeeType?: ProviderFeeType;
  providerFeeValue?: number;
  
  providerTermsAndConditions?: string;

  // Connection Access Settings
  connectionAccessOptions?: ConnectionAccessOption[];
  isFreeAccessFallbackEnabled?: boolean;
  freeAccessDurationMinutes?: number;
  disclaimerEmailContent?: string;
...
export interface ProviderApplication {
  ...
  overallRating?: number; // Added
  totalJobsCompleted?: number; // Added
}
...
export interface ConnectionAccessOption {
  id: 'oneTime' | 'sevenDays' | 'thirtyDays' | 'lifetime';
  label: string;
  price: number;
  durationDays?: number;
  enabled: boolean;
...
export interface UserProviderConnection {
    id?: string; // e.g., `${userId}_${providerId}`
    userId: string;
    providerId: string;
    accessType: 'free' | 'oneTime' | 'sevenDays' | 'thirtyDays' | 'lifetime';
    paymentId?: string;
    grantedAt: Timestamp;
...
export type ComplaintStatus = 'Pending' | 'Reviewed' | 'Closed';

export interface Complaint {
  id?: string; // Firestore document ID
  title: string;
  description: string;
  userId: string; // User who submitted the complaint
  userName: string;
  userEmail: string;
  userMobile: string;
  providerId: string; // Provider the complaint is against
  providerName: string;
